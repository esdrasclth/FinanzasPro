...
import FormDeuda from '@/app/components/FormDeuda'
import FormAbono from '@/app/components/FormAbono'
import Notificaciones from '@/app/components/Notificaciones'
import { calcularDeuda, ESTADO_META, type EstadoDeuda } from '@/app/lib/deudaCalculos'
...
export default function DeudasCliente({ deudasIniciales }: Props) {
  ...
  // La lista llega del Server Component; /api/deudas la refresca.
  const cargarDeudas = async () => {
    try {
      const res = await fetch('/api/deudas')
      if (!res.ok) return
      const json = await res.json()
      setDeudas((json.deudas || []).map((d: any) => ({ ...d, calc: calcularDeuda(d) })))
    } catch {
      ...
      <div className="max-w-[1728px] p-4 mx-auto sm:p-6 lg:p-8">
        ...
        <div className="flex items-start justify-between mb-5 sm:mb-8">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-obsidian">Controla tus compromisos</h1>
          </div>
