...
import FormCartera from '@/app/components/FormCartera'
import AjusteSaldo from '@/app/components/AjusteSaldo'
...
import { emitirCambio, useRecarga } from '@/app/lib/datos-bus'

interface Props {
  // Primer render servido desde el servidor: la pantalla ya llega con datos.
  carterasIniciales: any[]
  archivadasIniciales: any[]
}

export default function CarterasCliente({ carterasIniciales, archivadasIniciales }: Props) {
  ...
  // transacciones al navegador solo para sumarlas.
  ...
  const handleArchivar = async (cartera: any) => {
    const movs = Number(cartera.movimientos) || 0
    const mensaje = movs > 0
      ? `"${cartera.nombre}" se archivará y dejará de aparecer en la lista.\n\n` +
        `Sus ${movs} ${movs === 1 ? 'movimiento se conserva' : 'movimientos se conservan'} ` +
        `y siguen contando en el histórico y los reportes. Puedes restaurarla cuando quieras.`
      ...
      <div className="max-w-[1728px] p-4 mx-auto sm:p-6 lg:p-8">
        ...
        <div className="flex items-start justify-between mb-5 sm:mb-8">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-obsidian">Gestiona tu patrimonio</h1>
          </div>
        ...
        {/* Carteras archivadas: conservan su historial y se pueden restaurar */}
        {archivadas.length > 0 && (
          <div className="mb-6 border bg-snow border-fog rounded-card">
            ...
            >
              <span className="inline-flex items-center gap-2 text-sm font-medium text-graphite">
                <Archive size={16} strokeWidth={2} className="text-steel" />
                Carteras archivadas
                <span className="px-2 py-0.5 text-xs font-medium rounded-badge bg-mist text-steel">
                  {archivadas.length}
                </span>
              </span>
              <ChevronDown
                size={16}
                strokeWidth={2}
                className={`text-steel transition-transform ${mostrarArchivadas ? 'rotate-180' : ''}`}
              />
            </button>
            ...
            {mostrarArchivadas && (
              <div className="px-4 pb-4 space-y-2">
                <p className="mb-3 text-xs text-ash">
                  No aparecen en los totales de patrimonio, pero sus movimientos siguen
                  contando en el histórico y los reportes.
                </p>
                {archivadas.map(c => {
                  const Icono = ICONOS_TIPO[c.tipo] || Wallet
                  return (
                    <div
                      key={c.id}
                      className="flex items-center gap-3 p-3 border bg-mist/40 border-fog rounded-xl"
                    >
                      <span
                        className="flex items-center justify-center flex-shrink-0 w-9 h-9 rounded-xl"
                        style={{ backgroundColor: (c.color || '#71717a') + '15', color: c.color || '#71717a' }}
                      >
                        <Icono size={16} strokeWidth={2} />
                      </span>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate text-ink">{c.nombre}</p>
                        <p className="text-xs capitalize text-ash">{c.tipo} · {c.moneda || 'HNL'}</p>
                      </div>
                      <button
                        onClick={() => handleRestaurar(c)}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium transition-colors border rounded-full border-fog text-graphite hover:bg-snow hover:text-ink"
                      >
                        <RotateCcw size={13} strokeWidth={2} /> Restaurar
                      </button>
                      <button
