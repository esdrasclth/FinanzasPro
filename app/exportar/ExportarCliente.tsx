...
export default function ExportarCliente({ usuario, mesInicial, transaccionesIniciales, categoriasIniciales }: Props) {
  ...
  // El mes inicial llega del Server Component; al cambiarlo se pide de nuevo
  // reutilizando el buscador, que ya acepta rango de fechas.
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      ...
      const ultimo = new Date(parseInt(mes.slice(0, 4)), parseInt(mes.slice(5, 7)), 0).getDate()
      const finMes = `${mes}-${String(ultimo).padStart(2, '0')}`
      try {
        const res = await fetch(`/api/transacciones/buscar?desde=${inicioMes}&hasta=${finMes}&limit=500`)
