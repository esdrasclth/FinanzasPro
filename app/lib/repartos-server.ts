...
export interface RepartoPreparado {
  ...
  metodo: string
  fecha: Date
  participantes: { nombre: string; monto_asignado: number }[]
}
...
export function prepararReparto(body: any): PrepOk | PrepErr {
  ...
  const limpios = entradas
    .map(p => ({ nombre: String(p?.nombre || '').trim(), monto_asignado: Number(p?.monto_asignado) || 0 }))
    .filter(p => p.nombre.length > 0)

  if (limpios.length === 0) return { ok: false, status: 400, error: 'Agrega al menos un participante' }

  let participantes: { nombre: string; monto_asignado: number }[]

  if (metodo === 'igual') {
    const base = round2(montoTotal / limpios.length)
    participantes = limpios.map(p => ({ nombre: p.nombre, monto_asignado: base }))
    ...
    participantes = limpios.map(p => ({ nombre: p.nombre, monto_asignado: round2(p.monto_asignado) }))
  ...
  return {
    ok: true,
    data: { descripcion, montoTotal, moneda, metodo, fecha: new Date(fechaStr), participantes },
  }
}
