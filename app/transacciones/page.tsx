...
export default function Transacciones() {
  ...
  const handleDuplicar = async (t: any) => {
    if (tipoCompuesto(t)) {
      alert('Las transferencias y los abonos no se duplican desde aquí: registran movimiento en dos sitios a la vez. Créalos con el botón de nuevo movimiento.')
      return
    }
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return
